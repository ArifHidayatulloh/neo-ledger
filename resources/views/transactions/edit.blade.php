    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
        <form method="POST" action="{{ route('transactions.update', $transaction) }}" enctype="multipart/form-data">
            ...
            <div class="space-y-5">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Lampiran Saat Ini</label>
                    <div class="space-y-2">
                        @foreach($transaction->attachments as $attachment)
                            <div class="flex items-center justify-between p-2.5 bg-gray-50 dark:bg-gray-700 rounded-xl border border-gray-200 dark:border-gray-600">
                                <div class="flex items-center gap-2.5 min-w-0">
                                    @if(str_starts_with($attachment->file_type, 'image/'))
                                        <img src="{{ Storage::url($attachment->file_path) }}" alt="" class="w-10 h-10 object-cover rounded-lg flex-shrink-0">
                                    @else
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Tambah Lampiran <span class="text-gray-400">(opsional)</span></label>
                    <input type="file" name="attachments[]" multiple accept=".jpg,.jpeg,.png,.gif,.pdf,.doc,.docx,.xls,.xlsx"
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function filterCategories() {
        ...
        allOptions.filter(opt => opt.dataset.type === selectedType.value).forEach(opt => categorySelect.appendChild(opt));
    ...
    if (document.querySelector('input[name="type"]:checked')) filterCategories();
});
</script>
@endpush
